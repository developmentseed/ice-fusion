...
class StreamWeights(BaseModel):
    """Per-stream multipliers on the pixelwise log-likelihood. ``thick`` weights the thickness-change term; ``vel`` weights the velocity-change term."""  # noqa: E501
    thick: float = 1.5
    vel: float = 1.5
...
class SubsampleConfig(BaseModel):
    ...
    ``size`` is the maximum number of pixels per region; ``seed`` makes the draw deterministic. Use the same ``seed`` as the validation harness in ``PSUISM_HBM_V1`` to reproduce the prototype bit-exactly."""  # noqa: E501
    size: int = 20_000
    seed: int = 42


class InferenceConfig(BaseModel):
    """PyMC inference settings: weights on the per-stream likelihood, the subsample, and the standard NUTS knobs (``draws``, ``tune``, ``chains``, ``target_accept``)."""  # noqa: E501
...
class ProjectionConfig(BaseModel):
    """Which forward-projection target to compute (year + quantity) when applying the posterior weights."""  # noqa: E501
