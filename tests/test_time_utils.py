...
def test_decimal_years_decode_seconds_since():
...
def test_decimal_years_passthrough_when_already_years():
...
def test_clean_and_mask_drops_spurious_rows():
...
def test_year_snap_returns_none_outside_tol():
    available = np.array([2014, 2015, 2016])
